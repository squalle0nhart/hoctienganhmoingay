...
public class UnlockBar extends RelativeLayout {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        inflater.inflate(R.layout.unlock_bar_layout, this, true);
        ...
        // Get padding
        thumbWidth = dpToPx(50); // 60dp + 2*10dp
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            if (event.getX() > (sliderPosition - 100) && event.getX() < (sliderPosition + thumbWidth)) {
                sliding = true;
                initialSlidingX = event.getX();
                initialSliderPosition = sliderPosition;
            }
        } else if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_OUTSIDE) {
            if (sliderPosition >= (getMeasuredWidth() - thumbWidth - 100)) {
